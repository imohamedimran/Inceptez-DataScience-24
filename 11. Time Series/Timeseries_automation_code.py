...
data = pd.read_excel("Data.xlsx") #Read your decomposition file here
...
def Regression(data):
    ...
    Train=data[(data.Year>=2016)&(data.Year<2019)] #modify date according to your dataset; Train : 2017-2018
    ...
    Train["Level_index1"]=np.mean(Train[(Train.Year==2018)&
                          (Train.Quarter==1)]["D_Seasonalised_trend"])/np.mean(Train[(Train.Year==2017)
                          &(Train.Quarter==4)]["D_Seasonalised_trend"])
    
    numer1 = np.mean(Train[(Train.Year==2018)&(Train.Quarter==3)]["D_Seasonalised_trend"])/np.mean(Train[(Train.Year==2018)
           &(Train.Quarter==2)]["D_Seasonalised_trend"])
    numer2 = np.mean(Train[(Train.Year==2018)&(Train.Quarter==4)]["D_Seasonalised_trend"])/np.mean(Train[(Train.Year==2018)
           &(Train.Quarter==3)]["D_Seasonalised_trend"])
...
for model in [Regression, Arima, Holts_winter, Ses, Fbprophet]:
    for i in required_cols:
        data['Date'] = pd.to_datetime(data['Date'])
        data_new = data[["Date", "Variable_1", i]]
        data_new.columns=["Date","Variable_1", "Volume"]
        if model == Fbprophet:
            data_new.columns=["ds","Variable_1", "y"]
            Result_inter = model(data_new[['ds', 'y']])
            Result_inter.name = model.__name__ + "_" +  i
            Result_inter.index = range(len(Result_inter))
            Result = pd.concat([Result, Result_inter], axis = 1)
        elif model == Arima:
            Result_inter = model(data_new)
